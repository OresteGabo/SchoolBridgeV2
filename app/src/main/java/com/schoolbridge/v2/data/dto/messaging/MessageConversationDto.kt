 ...
 * You'll use `MessageConversationDto` primarily when:
 ...
 *
 * **How to use it:**
 * `MessageConversationDto` objects are received from your API layer. The nested `messages` list contains
 ...
 *
 * **Real-life Example:**
 * -   A parent creates a `MessageConversationDto` with the subject "Question about weekend event" to discuss an event.
 * -   A teacher replies within that thread.
 * -   Later, the parent wants to ask about homework, so they create a *new* `MessageConversationDto` for that new topic.
 *
 ...
 * @property messages A list of [MessageInConversationDto] objects representing the individual messages
 * within this thread, typically ordered chronologically. This list might be paginated or truncated
 * for large threads.
 */
data class MessageConversationDto(
    ...
    @SerializedName("messages") val messages: List<MessageInConversationDto>
)

/**
 * Data Transfer Object (DTO) for an **Individual Message within a Thread**.
 *
 * This DTO represents a single message that is part of a larger [MessageConversationDto].
 * It contains the sender, content, timestamp, and any embedded entity previews.
 * This DTO is typically nested within [MessageConversationDto] lists.
 *
 ...
 */
data class MessageInConversationDto(
    @SerializedName("id") val id: String,
